...
describe('WebhooksService', () => {
  ...
  beforeEach(() => {
    mockRepository = {
      createWebhook: jest.fn().mockImplementation((tenantId, data) =>
        Promise.resolve({
          id: 'webhook-123',
          name: data.name,
      ...
      logWebhookDelivery: jest
        .fn()
        .mockImplementation((tenantId, data) =>
          Promise.resolve({ id: 'del-123', success: data.success }),
        ),
        ...
        webhookEvent: {
          create: jest
            ...
            .mockImplementation((args) =>
              Promise.resolve({ id: 'event-123', ...args.data }),
            ),
        },
      },
    };
  ...
  it('should verify signatures and detect replay attacks', () => {
    ...
    const sig = require('../utils/signature.helper').generateWebhookSignature(
      secret,
      payload,
      timestamp,
    );
    ...
    const isOldValid = verifyWebhookSignature(
      secret,
      payload,
      sig,
      oldTimestamp,

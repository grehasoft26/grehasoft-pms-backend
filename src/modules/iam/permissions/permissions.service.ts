...
export class PermissionsService {
  ...
  async createGroup(dto: CreatePermissionGroupDto, context: RequestContext) {
    ...
    this.logger.audit(context.userId, 'Create Permission Group', 'permission_group', group, {
      ip: context.ip,
  ...
  // --- Categories ---
  async createCategory(dto: CreatePermissionCategoryDto, context: RequestContext) {
    ...
    this.logger.audit(context.userId, 'Create Permission Category', 'permission_category', category, {
      ip: context.ip,
      userAgent: context.userAgent,
      correlationId: context.correlationId,
      after: category,
  ...
  async createPermission(dto: CreatePermissionDto, context: RequestContext) {
    const categoryExists = await this.repository.findCategoryById(dto.categoryId);
    if (!categoryExists) {
      throw new NotFoundException('Permission Category not found');
    }
    const permissionExists = await this.repository.findPermissionByCode(dto.code);
    if (permissionExists) {
    ...
    this.logger.audit(context.userId, 'Create Permission', 'permission', permission, {
      ip: context.ip,
      userAgent: context.userAgent,
      correlationId: context.correlationId,
      after: permission,

...
import { CreateTaskDto, UpdateTaskDto, TaskFilterDto, CloneTaskDto } from './dto/tasks.dto';
...
export class TasksService {
  ...
  async create(dto: CreateTaskDto, context: RequestContext) {
    ...
    if (dto.parentTaskId) {
      ...
      if (!parent) throw new NotFoundException(`Parent task with ID ${dto.parentTaskId} not found`);
    }
    ...
    if (dto.isRecurring && dto.recurrenceRule) {
      nextRecurrenceDate = this.calculateNextRecurrence(new Date(), dto.recurrenceRule, dto.cronExpression);
    }
    ...
    // Publish events
    if (dto.assigneeIds && dto.assigneeIds.length > 0) {
      this.eventEmitter.emit('task.assigned', { taskId: task.id, assigneeIds: dto.assigneeIds });
    }
    this.eventEmitter.emit('task.status.changed', { taskId: task.id, beforeStatusId: null, afterStatusId: dto.statusId });

    // Audit Log
    this.logger.audit(context.userId, 'Create Task', 'task', task, { after: task });
  ...
  async update(id: string, dto: UpdateTaskDto, context: RequestContext) {
    ...
    const estimatedHours = dto.estimatedHours !== undefined ? dto.estimatedHours : Number(before.estimatedHours);
    const actualHours = dto.actualHours !== undefined ? dto.actualHours : Number(before.actualHours);
    const remainingHours = estimatedHours - actualHours >= 0 ? estimatedHours - actualHours : 0;

    let nextRecurrenceDate = before.nextRecurrenceDate;
    if (dto.isRecurring || dto.recurrenceRule) {
      const isRecur = dto.isRecurring !== undefined ? dto.isRecurring : before.isRecurring;
      const rule = dto.recurrenceRule !== undefined ? dto.recurrenceRule : before.recurrenceRule;
      const cron = dto.cronExpression !== undefined ? dto.cronExpression : before.cronExpression;
      if (isRecur && rule) {
        nextRecurrenceDate = this.calculateNextRecurrence(new Date(), rule, cron);
      } else {
    ...
    if (dto.assigneeIds !== undefined) {
      ...
      this.eventEmitter.emit('task.assigned', { taskId: id, assigneeIds: dto.assigneeIds });
    }
    ...
    this.logger.audit(context.userId, 'Update Task', 'task', updated, { before, after: updated });
    return this.getById(id);
  }

  // Kanban drag and drop position ordering (Refinement 7)
  async updatePosition(id: string, statusId: string, newPosition: number, context: RequestContext) {
    ...
    if (oldStatusId !== statusId) {
      ...
      await prisma.task.updateMany({
        where: { statusId: oldStatusId, position: { gt: oldPosition }, deletedAt: null },
  ...
  async delete(id: string, context: RequestContext) {
    ...
    this.logger.audit(context.userId, 'Delete Task', 'task', { id }, { before });
  }
  ...
  async restore(id: string, context: RequestContext) {
    ...
    this.logger.audit(context.userId, 'Restore Task', 'task', { id }, { before });
    return this.getById(id);
  }
  ...
  async permanentDelete(id: string, context: RequestContext) {
    ...
    this.logger.audit(context.userId, 'Permanent Delete Task', 'task', { id }, { before });
  }

  // Recurring Tasks Cron Scheduler (Refinement 8)
  @Cron('0 0 * * *') // Run daily at midnight
  async handleRecurringTasks() {
    this.logger.log('Running recurring tasks generation scheduler...', 'Scheduler');
    const now = new Date();
    ...
    for (const t of recurringTasks) {
      try {
        ...
        const nextDate = this.calculateNextRecurrence(new Date(), rule, t.cronExpression);

        await this.repository.update(t.id, {
          nextRecurrenceDate: nextDate,
        });

        this.logger.log(`Generated recurring task instance for Task Code: ${t.code}`, 'Scheduler');
      } catch (e) {
        this.logger.error(`Failed to process recurring task ${t.id}: ${e.message}`, 'Scheduler');
      }
    }
  }

  private calculateNextRecurrence(baseDate: Date, rule: string, cronExpr?: string | null): Date {

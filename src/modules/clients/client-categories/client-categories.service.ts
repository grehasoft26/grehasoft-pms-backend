...
export class ClientCategoriesService {
  ...
  async create(dto: CreateClientCategoryDto, context: RequestContext) {
    ...
    if (exists) throw new ConflictException(`Category with code "${dto.code}" already exists`);
    ...
    this.logger.audit(context.userId, 'Create Client Category', 'clientCategory', category, {
      ip: context.ip,
  ...
  async update(id: string, dto: UpdateClientCategoryDto, context: RequestContext) {
    ...
    this.logger.audit(context.userId, 'Update Client Category', 'clientCategory', updated, {
      ip: context.ip,
      userAgent: context.userAgent,
      correlationId: context.correlationId,
      before,
  ...
  async delete(id: string, context: RequestContext) {
    const before = await this.getById(id);
    if (before.isSystem) throw new ConflictException('System categories cannot be deleted');

    await this.repository.delete(id, context.userId);

    this.logger.audit(context.userId, 'Delete Client Category', 'clientCategory', { id }, {
      ip: context.ip,
      userAgent: context.userAgent,
      correlationId: context.correlationId,
      before,

...
export class RateLimiterService {
   ...
   */
  async checkLimit(key: string, maxAttempts: number, windowMs: number): Promise<void> {
  ...
  async checkLoginLimit(ip: string, email: string): Promise<void> {
    const max = this.configService.get<number>('app.auth.rateLimitLoginMax') || 5;
    const windowMs = this.configService.get<number>('app.auth.rateLimitLoginWindowMs') || 60000;
  ...
  async checkForgotLimit(ip: string): Promise<void> {
    const max = this.configService.get<number>('app.auth.rateLimitForgotMax') || 3;
    const windowMs = this.configService.get<number>('app.auth.rateLimitForgotWindowMs') || 900000;
  ...
  async checkResetLimit(ip: string): Promise<void> {
    const max = this.configService.get<number>('app.auth.rateLimitResetMax') || 3;
    const windowMs = this.configService.get<number>('app.auth.rateLimitResetWindowMs') || 900000;

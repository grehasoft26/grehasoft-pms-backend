...
export class KpisService {
  ...
  async calculateAndRecordKpi(
    ...
    if (!definition)
      throw new NotFoundException(`KPI Definition for code ${code} not found`);
    ...
    if (code === 'REV_GROWTH') {
      ...
      const won = opps.filter(
        (o) => o.probability === 100 || o.winReason,
      ).length;
      ...
      const billable =
        entries
          .filter((e) => e.billable)
          .reduce((s, e) => s + Number(e.duration || 0), 0) / 3600;
      const total =
        entries.reduce((s, e) => s + Number(e.duration || 0), 0) / 3600;
      ...
      const totalMin = incidents.reduce(
        (s, i) => s + (i.resolutionTime || 0),
        0,
      );
    ...
    const snapshot = await this.repository.recordKpiSnapshot(
      tenantId,
      definition.id,
      calculatedValue,
    );

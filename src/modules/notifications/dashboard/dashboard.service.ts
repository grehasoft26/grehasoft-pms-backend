...
export class DashboardService {
  ...
  async getStatistics(tenantId: string, userId: string) {
    ...
    const automationExecutions =
      await this.repository.prisma.automationExecution.count({
        where: { tenantId },
      });
    ...
    const approvedWorkflows =
      await this.repository.prisma.workflowExecution.count({
        where: { tenantId, status: 'APPROVED' },
      });
    const successRate =
      totalWorkflows > 0 ? (approvedWorkflows / totalWorkflows) * 100 : 100;

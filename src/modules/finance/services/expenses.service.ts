...
export class ExpensesService {
  ...
  async createExpense(dto: CreateExpenseDto, context: RequestContext) {
    ...
    this.logger.audit(context.userId, 'Create Expense', 'expense', expense, {
      after: expense,
    });
    return expense;
  }
  ...
  async updateStatus(
    ...
    if (
      ...
      currentStatus !== ExpenseStatus.SUBMITTED
    ) {
      throw new BadRequestException(
        'Expense must be Submitted to get Manager approval',
      );
    }
    if (
      status === ExpenseStatus.FINANCE_APPROVED &&
      currentStatus !== ExpenseStatus.MANAGER_APPROVED
    ) {
      throw new BadRequestException(
        'Expense must have Manager approval to get Finance approval',
      );
    }
    if (
      status === ExpenseStatus.PAID &&
      currentStatus !== ExpenseStatus.FINANCE_APPROVED
    ) {
      throw new BadRequestException(
        'Expense must be approved by Finance to trigger payment',
      );
    }
  ...
  async getExpenseCategories() {
    return this.repository.findExpenseCategories();
  }
}

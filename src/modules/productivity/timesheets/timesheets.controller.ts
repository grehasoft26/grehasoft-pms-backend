...
export class TimesheetsController {
  ...
  @ApiOperation({ summary: 'Submit weekly timesheet for approval (DRAFT -> SUBMITTED)' })
  @ApiResponse({ type: SuccessResponseDto })
  async submitTimesheet(@Body() dto: SubmitTimesheetDto, @Req() req: Request) {
    const context = this.getContext(req);
    const data = await this.service.submitTimesheet(context.userId, dto.startDate, context);
    return { message: 'Weekly timesheet submitted successfully', data };
  }

  @Patch(':id/approve')
  @Permissions('timesheets.approve')
  @ApiOperation({ summary: 'Approve or Reject weekly timesheet (SUBMITTED -> MANAGER_APPROVED -> FINANCE_APPROVED / REJECTED)' })
  @ApiResponse({ type: SuccessResponseDto })
  async approveTimesheet(
    ...
    const context = this.getContext(req);
    const data = await this.service.approveTimesheet(id, dto.status, dto.comments || '', context);
    return { message: `Timesheet state updated to ${dto.status}`, data };
  }

  @Get('pending')
  @Permissions('timesheets.approve')
  @ApiOperation({ summary: 'Get list of pending weekly timesheets awaiting approval' })
  @ApiResponse({ type: SuccessResponseDto })
  async getPending() {
  ...
  @ApiOperation({ summary: 'Get specific weekly timesheet details and approval history log' })
  @ApiResponse({ type: SuccessResponseDto })

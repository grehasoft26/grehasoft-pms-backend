...
export class TimeTrackingDashboardController {
  ...
  @ApiOperation({ summary: 'Get active working/idle users, today/weekly hours and top productivity ranks' })
  @ApiResponse({ type: SuccessResponseDto })
  async getStats() {
    const data = await this.service.getDashboardStats();
    return { message: 'Time Tracking Dashboard metrics retrieved successfully', data };
  }
}

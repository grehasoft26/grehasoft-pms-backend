...
export class HostingController {
  ...
  @Post('accounts')
  @Permissions('infrastructure.manage')
  @ApiOperation({ summary: 'Create a client hosting account (Disk limits, bandwidth limits)' })
  @ApiResponse({ type: SuccessResponseDto })
  async createAccount(@Body() dto: CreateHostingAccountDto, @Req() req: Request) {
    const context = this.getContext(req);
  ...
  @ApiResponse({ type: SuccessResponseDto })
  async getAccounts(@Query('clientId') clientId?: string, @Query('projectId') projectId?: string) {
  ...
  @Get('accounts/:id')
  @Permissions('infrastructure.read')
  @ApiOperation({ summary: 'Get hosting account details with plans & credentials' })
  @ApiResponse({ type: SuccessResponseDto })

...
export class KeywordsService {
  ...
  async addKeyword(
    ...
    if (
      termLower.includes('buy') ||
      termLower.includes('pricing') ||
      termLower.includes('purchase')
    ) {
      intent = 'TRANSACTIONAL';
    } else if (
      termLower.includes('vs') ||
      termLower.includes('review') ||
      termLower.includes('best')
    ) {
      intent = 'COMMERCIAL';
    ...
    await this.repository.logAudit(
      tenantId,
      'Create Keyword',
      `Keyword ${dto.term} added to project.`,
    );
    return keyword;
  }
  ...
  async runKeywordClustering(tenantId: string, seoProjectId: string) {
    ...
    for (const cluster of clusters) {
      ...
      const matchIds = keywords
        .filter((k) => cluster.terms.includes(k.term))
        .map((k) => k.id);
    ...
    await this.repository.logAudit(
      tenantId,
      'Keyword Clustering',
      `Clustered keywords into ${clusters.length} silos.`,
    );
    return clusters;
  }
}
